...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if google_json.exists() {
        ...
        embed_creds(
            &google_json,
            "iriebook/src/resource_access/google_auth.rs",
    ...
    if github_json.exists() {
        ...
        embed_creds(
            &github_json,
            "iriebook/src/resource_access/github_auth.rs",
            "GITHUB_CREDENTIALS_B64",
        )?;
    } else {
...
fn embed_creds(
    ...
    for line in lines.iter_mut() {
        if line
            .trim()
            .starts_with(&format!("const {}: &str =", const_name))
        {
    ...
    if !found {
        return Err(format!(
            "Could not find constant {} in {} (line matching failed)",
            const_name, rs_path
        )
        .into());
    }
